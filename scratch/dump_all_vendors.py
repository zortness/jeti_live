        ...
        for i, pkt in enumerate(packets):
            ...
            if transfer_type == 2: # URB_CONTROL
                if len(pkt) >= header_len + 8:
                    ...
                    unique_reqs.add((req_type, request))
                    
        print(f"--- Unique Control requests in {pcap_file} ---")
        for req_type, request in sorted(unique_reqs):
            print(f"  ReqType: 0x{req_type:02X}, Request: 0x{request:02X}")
